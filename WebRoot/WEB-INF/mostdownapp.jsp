...
<div class="struts">
  <div class="app_table">
  		<table width="100%">
  			<c:forEach items="${mostdownapplist}" var="app">
  			<tr>
    			<td><div class="app_icon"><a href="${pageContext.request.contextPath }/showApp.do?flag=showapp&id=${app.id}"><img style="width: 65px; height: 65px;border: 0" src="images/game/${app.appSrc }/App_logo/${app.appLogo}"/></a></div>
					...
					<div style="margin-left: 200px; margin-top: 30px;font-family: '微软雅黑'">${app.appDownnum}下载</div>
 ...
 <c:if test="${pageNow-1>0}">
  <div class="shangyiye"><a href="${pageContext.request.contextPath}/apptype.do?flag=mostdownload&pageNow=${pageNow-1}">上一页</a></div>
  	</c:if>	
  		<div class="yema"><a href="${pageContext.request.contextPath}/apptype.do?flag=mostdownload&pageNow=1">首页</a></div>
  <c:if test="${pageNow<pageCount}">
  <div class="xiayiye"><a href="${pageContext.request.contextPath}/apptype.do?flag=mostdownload&pageNow=${pageNow+1}">下一页</a></div>
  </c:if>		
 </div>
</div>
