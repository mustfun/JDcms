...
<body>
	...
	<div class="houtai"><div style="margin-top:40px; font-size:36px; color:#0000FF; margin-left:550px;">用户管理中心</div></div>
	...
	<div class="shujuqu">
			...
			<table width="100%" border="0">
				 ...
				 <c:forEach items="${userlist}" var="user">
				  <tr bgcolor="#F5F7F7" style="height: 50px;">
					...
					<td>${user.oicq }</td>
					<td>${user.email }</td>
					<td><a onclick="return confirm('你真的要删除吗');" href="${pageContext.request.contextPath}/admin.do?flag=delsomedody&type=user&id=${user.id}">删除</a></td>
	  ...
	  <c:if test="${pageNow>1}">
	  <a href="${pageContext.request.contextPath}/admin.do?flag=shownextuser&pageNow=${pageNow-1}">上一页</a></c:if></span>
	  <span style=" margin-left:10px;color:#00DFFF; font-weight:bolder">
	  <c:if test="${pageNow<pageCount}">
	  <a href="${pageContext.request.contextPath}/admin.do?flag=shownextuser&pageNow=${pageNow+1}">下一页</a></c:if></span></div>
